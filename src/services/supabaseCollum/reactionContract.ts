...
export const createReactionContract = async (contract: ReactionContract) => {
  ...
  try {
    ...
    console.warn("Could not fetch licensor profile for auto-accept check", e);
  }
...
export const adminDeleteReactionContract = async (id: string) => {
  ...
  try {
    ...
    console.error(
      "Admin: Failed to cancel Stripe subscription before deletion:",
      e,
    );
  }
  ...
  if (checkData) {
    throw new Error(
      "Missing Permission: Supabase Row Level Security (RLS) prevented the deletion. Please ensure the admin role has delete access on reaction_contracts.",
    );
  }
...
export const adminReportUsage = async (
  contractId: string,
  mockViews: number,
) => {
