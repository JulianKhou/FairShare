...
export const MyLicenses = () => {
  ...
  const getStatusBadge = (license: ReactionContract) => {
    ...
    if (["REJECTED", "CANCELLED", "PAYMENT_FAILED", "EXPIRED"].includes(s)) {
      return (
        <Badge
          variant="outline"
          className="text-red-600 border-red-500 bg-red-50 px-3 py-1 text-sm flex items-center gap-1.5"
        >
  ...
  // Filter counts
  const INACTIVE = ["CANCELLED", "REJECTED", "PAYMENT_FAILED", "EXPIRED"];
  const counts = {
    all: licenses.length,
    one_time: licenses.filter(
      (l) => l.pricing_model_type === 1 && !INACTIVE.includes(l.status || ""),
    ).length,
    subscription: licenses.filter(
      (l) => l.pricing_model_type !== 1 && !INACTIVE.includes(l.status || ""),
    ).length,
    rejected: licenses.filter((l) => INACTIVE.includes(l.status || "")).length,
  };
  ...
  const filteredLicenses = licenses.filter((l) => {
    ...
    if (activeFilter === "one_time")
      return l.pricing_model_type === 1 && !INACTIVE.includes(s);
    if (activeFilter === "subscription")
      return l.pricing_model_type !== 1 && !INACTIVE.includes(s);
    if (activeFilter === "rejected") return INACTIVE.includes(s);
    return true;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {(
          [
            { key: "all", label: "Alle Lizenzen" },
            { key: "one_time", label: "Einmalzahlung" },
            { key: "subscription", label: "Abos (Views-basiert)" },
            { key: "rejected", label: "Abgelehnt / Abgelaufen" },
          ] as { key: LicenseFilter; label: string }[]
        ).map(({ key, label }) => (
          <Button
            key={key}
      ...
      {licenses.length === 0 ? (
        ...
        <p className="text-center text-muted-foreground py-8">
          Keine Lizenzen für diesen Filter.
        </p>
      ) : (
              ...
              <CardContent className="p-6">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-border">
                  <div className="flex flex-col gap-1 text-sm">
                    {licenseSpents[license.id] !== undefined &&
                    licenseSpents[license.id] > 0 ? (
                      <>
                        <div className="flex items-center gap-2">
                          <span className="uppercase tracking-wider text-xs font-semibold text-emerald-600">
                            Bisher gezahlt:
                          </span>
